...
This is `src.source_figure.plot_source` with two extra markers — the figure
...
def main():
    ...
    for n, (_, row) in enumerate(rows.iterrows(), 1):
        sid = row['id']
        markers = spt_markers(row)
        csc2 = csc2_marker(row)
        if csc2:
            markers.append(csc2)
        else:
        ...
        _, failures = plot_source(row, out_path=out, markers=markers,
                                  fov_arcsec=args.fov)
